...
describe("LoginForm", () => {
  it("affiche les champs email, mot de passe et le bouton", () => {
    ...
    expect(screen.getByRole("link", { name: /mot de passe oublié/i })).toHaveAttribute("href", "/fr/forgot-password")
  })
  ...
  it("appelle login() avec les identifiants et redirige en cas de succès", async () => {
    ...
    await waitFor(() => expect(loginMock).toHaveBeenCalledWith("yannick@test.com", "Secret123!"))
  ...
  it("affiche le message d'erreur renvoyé par login() et ne redirige pas", async () => {
    loginMock.mockResolvedValue({ success: false, error: "Email ou mot de passe incorrect" })
    const user = userEvent.setup()
    render(<LoginForm />)
  ...
  it("désactive le bouton pendant la connexion", async () => {
    ...
    const pending = await screen.findByRole("button", { name: /connexion\.\.\./i })
