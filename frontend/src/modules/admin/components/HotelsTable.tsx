...
export default function HotelsTable() {
  ...
  useEffect(() => {
    ...
    const fetchHotels = async () => {
      ...
      try {
        ...
        if (!cancelled)
          setError("Impossible de charger les hôtels. Réessayez.");
      } finally {
  ...
  const openHotel = (id: number) =>
    router.push(`/${countryCode}/admin/hotels/${id}`);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <span className="text-sm text-gris-moyen">
          {pagination
            ? `${pagination.total} hôtel${pagination.total > 1 ? "s" : ""}`
            : ""}
        </span>
        <Link
            ...
            {!loading &&
              ...
              hotels.map((hotel) => (
                <tr
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && openHotel(hotel.id_hotel)
                  }
                  tabIndex={0}
                  className="cursor-pointer hover:bg-turquoise-light focus:outline-none focus:bg-turquoise-light"
                >
                  <td className="px-4 py-3 text-gris-moyen">
                    {hotel.id_hotel}
                  </td>
                  <td className="px-4 py-3 font-medium text-gris-fonce">
                    {hotel.nom_hotel}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    {hotel._count.chambres}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span
                      className={
                        hotel._count.reservations > 0
                          ? "font-semibold text-gris-fonce"
                          : "text-gray-400"
                      }
                    >
