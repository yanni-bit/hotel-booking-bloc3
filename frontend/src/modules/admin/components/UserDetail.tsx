...
interface User {
  ...
  role: Role;
  adresse: {
    rue_user: string | null;
    complement_user: string | null;
    code_postal_user: string | null;
    ville_user: string | null;
    pays_user: string | null;
  } | null;
  reservations: {
...
export default function UserDetail({ userId, countryCode }: UserDetailProps) {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled)
          setMessage({
            type: "error",
            text: "Impossible de charger l'utilisateur.",
          });
      } finally {
  ...
  const patch = async (
    body: { id_role?: number; actif?: boolean },
    okText: string,
  ) => {
    setSaving(true);
    setMessage(null);
    try {
      ...
      if (!response.ok) {
        setMessage({
          type: "error",
          text: data.error ?? "Modification impossible.",
        });
        return;
      }
      setUser((prev) =>
        prev ? { ...prev, role: data.user.role, actif: data.user.actif } : prev,
      );
      ...
      setMessage({
        type: "error",
        text: "Modification impossible. Réessayez.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm(
        `Supprimer le compte de ${user.prenom_user} ${user.nom_user} ? Cette action est irréversible.`,
      )
    )
      return;
    setMessage(null);
    try {
      ...
      if (!response.ok) {
        ...
        setMessage({
          type: "error",
          text: data.error ?? "Suppression impossible.",
        });
        return;
      }
      router.push(listUrl);
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gris-fonce">
              ...
              {isSelf && (
                <span className="ml-2 text-sm font-normal text-gris-moyen">
                  (vous)
                </span>
              )}
            </h1>
            <p className="text-sm text-gris-moyen">
              {u.email_user} · ID {u.id_user}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) =>
                patch({ id_role: parseInt(e.target.value) }, "Rôle mis à jour.")
              }
          ...
          <button
            ...
            onClick={() =>
              patch(
                { actif: !u.actif },
                u.actif ? "Compte désactivé." : "Compte réactivé.",
              )
            }
            className={`px-4 py-2 rounded-rounded text-sm border disabled:opacity-50 ${
              u.actif
                ? "border-gray-300 text-gris-fonce hover:bg-white"
                : "border-validation text-validation hover:bg-green-50"
            }`}
          >
                    ...
                    {u.reservations.map((r) => (
                      <tr key={r.id_reservation}>
                        <td className="py-2 pr-4">
                          <Link
                            href={`/${countryCode}/admin/reservations/${r.id_reservation}`}
                            className="font-mono text-xs text-turquoise hover:underline"
                          >
                ...
                {u.avis.map((a) => (
                  <li key={a.id_avis} className="py-3 text-sm">
                    <div className="flex justify-between gap-4">
                      <Link
                        href={`/${countryCode}/admin/hotels/${a.hotel.id_hotel}`}
                        className="font-medium text-turquoise hover:underline"
                      >
                        {a.hotel.nom_hotel}
                      </Link>
                      <span className="text-gris-moyen whitespace-nowrap">
                        {Number(a.note).toFixed(1)}/10 ·{" "}
                        {formatDate(a.date_avis)}
                      </span>
                    </div>
                    {a.titre_avis && (
                      <p className="mt-1 font-medium text-gris-fonce">
                        {a.titre_avis}
                      </p>
                    )}
                    {a.commentaire && (
                      <p className="mt-1 text-gris-fonce">{a.commentaire}</p>
                    )}
                  </li>
                ))}
              </ul>
        ...
        <div className="space-y-6">
          <Section title="Compte">
            <dl className="space-y-2">
              ...
              <Row
                label="Email vérifié"
                value={u.email_verifie ? "Oui" : "Non"}
              />
              <Row label="Inscrit le" value={formatDate(u.date_inscription)} />
              <Row
                label="Dernière connexion"
                value={formatDateTime(u.derniere_connexion)}
              />
            </dl>
          </Section>
                ...
                <Row
                  ...
                  value={
                    <span className="whitespace-pre-line">
                      {[
                        u.adresse.rue_user,
                        u.adresse.complement_user,
                        [u.adresse.code_postal_user, u.adresse.ville_user]
                          .filter(Boolean)
                          .join(" "),
                        u.adresse.pays_user,
                      ]
