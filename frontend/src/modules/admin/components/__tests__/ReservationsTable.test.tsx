...
const listResponse = {
  reservations: [
    {
      id_reservation: 10, num_confirmation: "HB-TEST-0001",
      check_in: "2026-09-16", check_out: "2026-09-18", nbre_nuits: 2,
      total_price: "156.00", devise: "EUR", date_reservation: "2026-09-10T10:00:00Z",
      user: { id_user: 6, nom_user: "Franchaisse", prenom_user: "Yannick", email_user: "y@test.com" },
      hotel: { id_hotel: 98, nom_hotel: "Luxury Tokyo Hotel", ville_hotel: "Tokyo" },
...
beforeEach(() => {
  ...
  global.fetch = jest.fn((url: string, init?: RequestInit) => {
    if (init?.method === "PUT") return jsonResponse({ reservation: { id_reservation: 10, statut: statuts[1] } })
...
describe("ReservationsTable", () => {
  ...
  it("pré-filtre sur le statut reçu en prop", async () => {
    ...
    await screen.findByText("HB-TEST-0001")
    expect(global.fetch).toHaveBeenCalledWith("/api/admin/reservations?page=1&limit=15&statut=1")
  })
  ...
  it("change le statut en ligne et envoie un PUT", async () => {
    ...
    const select = screen.getByLabelText(/changer le statut de la réservation HB-TEST-0001/i)
    await user.selectOptions(select, "2")

    // Mise à jour optimiste : le badge change tout de suite
    const row = screen.getByText("HB-TEST-0001").closest("tr")!
    expect(within(row).getByText("Confirmée", { selector: "span" })).toBeInTheDocument()

    await waitFor(() =>
      expect(global.fetch).toHaveBeenCalledWith(
        "/api/admin/reservations/10",
        expect.objectContaining({ method: "PUT", body: JSON.stringify({ id_statut: 2 }) })
      )
    )
  })
  ...
  it("revient au statut précédent si le PUT échoue", async () => {
    global.fetch = jest.fn((url: string, init?: RequestInit) => {
      if (init?.method === "PUT") return jsonResponse({ error: "Erreur" }, false, 500)
      return jsonResponse(listResponse)
    }) as jest.Mock
    ...
    const row = screen.getByText("HB-TEST-0001").closest("tr")!
    await waitFor(() => expect(within(row).getByText("En attente", { selector: "span" })).toBeInTheDocument())
