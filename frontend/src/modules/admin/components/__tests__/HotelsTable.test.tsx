  ...
  hotels: [
    {
      ...
      nbre_etoile_hotel: 5,
      note_moy_hotel: "9.4",
      _count: { chambres: 4, reservations: 2 },
    },
    {
      id_hotel: 2,
      nom_hotel: "Tokyo Inn",
      ...
      note_moy_hotel: null,
      _count: { chambres: 3, reservations: 0 },
    },
  ],
...
describe("HotelsTable", () => {
  ...
  it("appelle l'API avec la page et la limite", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "/api/admin/hotels?page=1&limit=15",
    );
  });
  ...
  it("relance la recherche avec un délai après la saisie", async () => {
    ...
    await waitFor(() =>
      expect(global.fetch).toHaveBeenLastCalledWith(
        "/api/admin/hotels?page=1&limit=15&search=paris",
      ),
    );
  });
  ...
  it("passe à la page suivante", async () => {
    ...
    await waitFor(() =>
      expect(global.fetch).toHaveBeenLastCalledWith(
        "/api/admin/hotels?page=2&limit=15",
      ),
    );
  });
  ...
  it("affiche un message d'erreur si l'API échoue", async () => {
    global.fetch = jest.fn(() =>
      jsonResponse({ error: "Non autorisé" }, false, 401),
    ) as jest.Mock;
    render(<HotelsTable />);

    expect(
      await screen.findByText(/impossible de charger les hôtels/i),
    ).toBeInTheDocument();
  });
  ...
  it("affiche un message quand aucun hôtel ne correspond", async () => {
    ...
    expect(
      await screen.findByText(/aucun hôtel ne correspond/i),
    ).toBeInTheDocument();
  });
});
