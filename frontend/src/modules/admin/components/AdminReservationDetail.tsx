...
export default function AdminReservationDetail({
  ...
  const handleStatutChange = async (newStatutId: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage({
          type: "error",
          text: data.error ?? "Changement de statut impossible.",
        });
        return;
      }
      setReservation((prev) =>
        prev ? { ...prev, statut: data.reservation.statut } : prev,
      );
      setMessage({
        type: "ok",
        text: `Statut passé à « ${data.reservation.statut.nom_statut} ».`,
      });
    } catch (err) {
      console.error("Erreur changement statut:", err);
      setMessage({
        type: "error",
        text: "Changement de statut impossible. Réessayez.",
      });
    } finally {
      setSaving(false);
    }
  ...
  if (notFound || !reservation) {
    return (
      <div className="space-y-4">
        ...
        <Link href={listUrl} className="text-turquoise hover:underline">
          Retour à la liste
        </Link>
      </div>
    );
  ...
  const r = reservation;
  const totalServices = r.services.reduce(
    (sum, s) => sum + Number(s.sous_total),
    0,
  );
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gris-fonce font-mono">
              {r.num_confirmation ?? `Réservation #${r.id_reservation}`}
            </h1>
            <p className="text-sm text-gris-moyen">
              Créée le {formatDateTime(r.date_reservation)}
            </p>
          </div>
        </div>
      ...
      {message && (
        <p
          role="status"
          className={`px-4 py-3 rounded-rounded text-sm ${message.type === "ok" ? "bg-green-50 text-validation" : "bg-red-50 text-erreur"}`}
        >
          {message.text}
        </p>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Section title="Séjour">
            <dl className="space-y-2">
              <Row
                ...
                value={
                  <Link
                    href={`/${countryCode}/admin/hotels/${r.hotel.id_hotel}`}
              ...
              <Row
                label="Lieu"
                value={`${r.hotel.ville_hotel}, ${r.hotel.pays_hotel}`}
              />
              ...
              <Row
                ...
                value={
                  r.offre
                    ? `${r.offre.nom_offre}${r.offre.petit_dejeuner_inclus ? " · petit-déjeuner inclus" : ""}${r.offre.remboursable || /remboursable/i.test(r.offre.nom_offre) ? "" : " · non remboursable"}`
                    : "—"
                }
              />
              ...
              <Row
                label="Durée"
                value={`${r.nbre_nuits} nuit${r.nbre_nuits > 1 ? "s" : ""}`}
              />
              <Row
                label="Voyageurs"
                value={`${r.nbre_adults} adulte${r.nbre_adults > 1 ? "s" : ""}${r.nbre_children > 0 ? `, ${r.nbre_children} enfant${r.nbre_children > 1 ? "s" : ""}` : ""}`}
              />
              {r.cancel_deadline && (
                <Row
                  label="Annulation gratuite jusqu'au"
                  value={formatDate(r.cancel_deadline)}
                />
              )}
            </dl>
            {r.special_requests && (
          ...
          <Section title="Montants">
            <dl className="space-y-2">
              <Row
                label={`Chambre (${r.nbre_nuits} × ${money(r.prix_nuit, r.devise)})`}
                value={money(totalChambre, r.devise)}
              />
              {r.services.map((s) => (
                <Row
                  key={s.id_reservation_service}
              ...
              {r.services.length > 0 && (
                <Row
                  ...
                  value={money(totalServices, r.devise)}
                />
              )}
              <div className="flex justify-between pt-3 mt-1 border-t border-gray-200 text-base font-bold">
                ...
                <dd className="text-turquoise">
                  {money(r.total_price, r.devise)}
                </dd>
              </div>
            </dl>
          </Section>
          ...
          <Section title="Client">
            <dl className="space-y-2">
              <Row
                ...
                value={
                  <Link
                    href={`/${countryCode}/admin/users/${r.user.id_user}`}
                    className="text-turquoise hover:underline"
                  >
          ...
          <Section title="Paiement">
            {r.paiement ? (
              <dl className="space-y-2">
                <Row
                  label="Montant"
                  value={money(r.paiement.montant, r.devise)}
                />
                <Row
                  label="Méthode"
                  value={r.paiement.methode_paiement ?? "—"}
                />
                ...
                <Row
                  label="Date"
                  value={formatDateTime(r.paiement.date_paiement)}
                />
              </dl>
            ) : (
              <p className="text-sm text-gris-moyen">
