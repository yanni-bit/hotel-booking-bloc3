...
interface Reservation {
  ...
  user: { id_user: number; nom_user: string; prenom_user: string; email_user: string }
...
export default function ReservationsTable({ initialStatut = "all" }: ReservationsTableProps) {
  ...
  useEffect(() => {
    ...
    const fetchReservations = async () => {
      ...
      try {
        ...
        if (!cancelled) setError("Impossible de charger les réservations. Réessayez.")
      } finally {
  ...
  const handleStatutChange = async (reservationId: number, newStatutId: number) => {
    const previous = reservations
    const newStatut = statuts.find((s) => s.id_statut === newStatutId) ?? null
    setReservations((list) =>
      list.map((r) => (r.id_reservation === reservationId ? { ...r, statut: newStatut } : r))
    )
  ...
  const openReservation = (id: number) => router.push(`/${countryCode}/admin/reservations/${id}`)
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <span className="text-sm text-gris-moyen">
          {pagination ? `${pagination.total} réservation${pagination.total > 1 ? "s" : ""}` : ""}
        </span>
      </div>
      ...
      <div className="bg-white rounded-rounded border border-gray-200 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading && (
              <tr><td colSpan={7} className="px-4 py-10 text-center text-gris-moyen">Chargement…</td></tr>
            )}
            {!loading && reservations.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-10 text-center text-gris-moyen">Aucune réservation ne correspond.</td></tr>
            )}
            {!loading && reservations.map((r) => (
              <tr
                key={r.id_reservation}
                onClick={() => openReservation(r.id_reservation)}
                onKeyDown={(e) => e.key === "Enter" && openReservation(r.id_reservation)}
                tabIndex={0}
                className="cursor-pointer hover:bg-turquoise-light focus:outline-none focus:bg-turquoise-light"
              >
                <td className="px-4 py-3 font-mono text-xs text-gris-fonce">{r.num_confirmation ?? `#${r.id_reservation}`}</td>
                <td className="px-4 py-3">
                  <p className="font-medium text-gris-fonce">{r.user.prenom_user} {r.user.nom_user}</p>
                  <p className="text-xs text-gris-moyen">{r.user.email_user}</p>
                </td>
                <td className="px-4 py-3">
                  <p className="font-medium text-gris-fonce">{r.hotel.nom_hotel}</p>
                  <p className="text-xs text-gris-moyen">{r.hotel.ville_hotel}</p>
                </td>
                <td className="px-4 py-3">{r.chambre.type_room}</td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {formatDate(r.check_in)} → {formatDate(r.check_out)}
                  <span className="block text-xs text-gris-moyen">{r.nbre_nuits} nuit{r.nbre_nuits > 1 ? "s" : ""}</span>
                </td>
                <td className="px-4 py-3 text-right font-medium whitespace-nowrap">
                  {Number(r.total_price).toFixed(2)} {r.devise === "EUR" ? "€" : r.devise}
                </td>
                ...
                <td className="px-4 py-3" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) => handleStatutChange(r.id_reservation, parseInt(e.target.value))}
