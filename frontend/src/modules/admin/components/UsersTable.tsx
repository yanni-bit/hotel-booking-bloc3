...
export default function UsersTable() {
  ...
  const patchUser = async (
    userId: number,
    patch: { id_role?: number; actif?: boolean },
  ) => {
    ...
    setUsers((list) =>
      list.map((u) => {
        ...
        const role =
          patch.id_role !== undefined
            ? (roles.find((r) => r.id_role === patch.id_role) ?? u.role)
            : u.role;
    ...
    try {
      ...
      setUsers(previous);
      setError(
        err instanceof Error ? err.message : "La modification a échoué.",
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  const openUser = (id: number) =>
    router.push(`/${countryCode}/admin/users/${id}`);
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-gris-fonce">
                        ...
                        {isSelf && (
                          <span className="ml-2 text-xs text-gris-moyen">
                            (vous)
                          </span>
                        )}
                      </p>
                    ...
                    >
                      <button
                        ...
                        disabled={busy || isSelf}
                        onClick={() =>
                          patchUser(u.id_user, { actif: !u.actif })
                        }
