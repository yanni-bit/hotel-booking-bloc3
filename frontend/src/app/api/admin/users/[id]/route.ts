...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (typeof data.nom_user === "string" && data.nom_user.trim())
      updateData.nom_user = data.nom_user.trim();
    if (typeof data.prenom_user === "string" && data.prenom_user.trim())
      updateData.prenom_user = data.prenom_user.trim();
    if (typeof data.email_user === "string" && data.email_user.trim())
      updateData.email_user = data.email_user.trim();
    if (data.tel_user !== undefined)
      updateData.tel_user = data.tel_user || null;
    if (typeof data.actif === "boolean") updateData.actif = data.actif;
    if (typeof data.email_verifie === "boolean")
      updateData.email_verifie = data.email_verifie;

    if (data.id_role !== undefined) {
      const idRole = Number(data.id_role);
      const roleExiste =
        Number.isInteger(idRole) &&
        (await prisma.role.findUnique({ where: { id_role: idRole } }));
      if (!roleExiste)
