...
// Forme d'une offre telle que renvoyée par Prisma avec le `select` ci-dessous.
// `prix_nuit` est un Decimal Prisma : on le convertit en number avant usage.
type OffrePrisma = {
  id_offre: number;
  nom_offre: string | null;
  description_offre: string | null;
  prix_nuit: Prisma.Decimal;
  devise: string | null;
